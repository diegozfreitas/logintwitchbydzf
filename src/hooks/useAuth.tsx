  ...
  async function signIn() {
    try {
      ...
      if (
        ...
        const useResponse = await api.get("/users");

        console.log(useResponse);

        setUser({
          id: useResponse?.data?.data[0]?.id,
          display_name: useResponse?.data?.data[0]?.display_name,
          email: useResponse?.data?.data[0]?.email,
          profile_image_url: useResponse?.data?.data[0]?.profile_image_url,
        });
